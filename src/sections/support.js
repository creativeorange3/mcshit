...
import support from 'assets/images/realshit.png';
...
const data = [
  {
    id: 1,
    icon: support,
    title: '5% Liquidity tax',
    description: `Good liquidity tax to ensure that chart doesn't dump to oblivion once knowledgeable investors realize that we won't be able to afford Caesar Calls and sell their $10 bags`,
  },
  {
    id: 2,
    icon: support,
          ...
          {data?.map((item) => (
            <Flex key={item.id} sx={styles.supportItem}>
              <Flex as="figure" sx={styles.media}>
                <Image src={item?.icon} alt={item?.title} style={{maxWidth: '80px'}}/>
              </Flex>
